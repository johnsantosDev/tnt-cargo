...
<div class="container">
    ...
    <table class="summary-table">
        <tr>
            ...
            <td><div class="label">Moyenne Dette</div><div class="value">${{ number_format($avgDebt, 2, ',', ' ') }}</div></td>
        </tr>
    </table>
    <table class="data-table">
        <thead><tr><th>#</th><th>Client</th><th>Téléphone</th><th>Dette</th><th>Expéditions</th></tr></thead>
        <tbody>
        @foreach($debtByClient as $i => $d)
            <tr>
                ...
                <td>{{ $d['name'] }}</td>
                <td>{{ $d['phone'] }}</td>
                <td class="red">${{ number_format($d['total_debt'], 2, ',', ' ') }}</td>
                <td>{{ $d['shipment_count'] }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
